...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask(){
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        double sumWorkingOnTask = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t-> LocalDate.now().until(t.getCreated(), DAYS))
                .reduce(0L, (sum, current)-> sum+ current);

        List<Task> countWorkingOnTask = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .collect(Collectors.toList());
        double average = -sumWorkingOnTask/countWorkingOnTask.size();
    ...
    @Test
    public void testAddTaskListAverageWorkingOnTaskVol2() {
        //Given
        Board project = prepareTestData();
